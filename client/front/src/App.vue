...
async function refreshDashboard(autoInitMissing = false) {
  ...
  await Promise.all([fetchTasks(), fetchLogs(), fetchCapabilities()])
    ...
    <div class="shell">
      ...
      <section v-else class="workspace">
        ...
        <div class="card log-card">
          ...
          <div v-for="item in logs.slice(0, 18)" :key="item.id" class="log-row">
            <span class="log-level" :class="item.level === 'error' ? 'error' : 'info'">{{ item.level }}</span>
            <span>{{ item.taskName }} {{ item.filePath || '' }} {{ item.message }}</span>
          </div>
        </div>
      </section>
